...
import 'package:fabrics_app/Models/menu.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  _goMenu(String? nombre) {
    if (nombre == 'Orden') {
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (context) =>
              OrderNewScreen(orden: order, user: widget.user, isOld: true),
        ),
      );
      return;
    }
    ...
    if (nombre == 'Nuevo Pedido') {
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (context) =>
              OrderNewScreen(orden: order, user: widget.user, isOld: false),
        ),
      );
    }
  ...
  void _getMenus() {
    Menu menu = Menu(
      nombre: 'Nuevo Pedido',
      assetV: 'assets/Cart.png',
      color: const Color.fromARGB(255, 0, 17, 14).withOpacity(0.7),
    );
    menus.add(menu);

    menu = Menu(
      nombre: 'Pedidos',
      assetV: 'assets/pedidos.png',
      color: const Color.fromARGB(255, 39, 4, 4).withOpacity(0.7),
    );
    menus.add(menu);
    ...
    menu = Menu(
      ...
      assetV: 'assets/Rollos6.png',
      color: const Color.fromARGB(255, 46, 3, 39).withOpacity(0.7),
    );
    menus.add(menu);

    menu = Menu(
      nombre: 'Inventario',
      assetV: 'assets/rollos1.png',
      color: const Color.fromARGB(255, 2, 28, 70).withOpacity(0.7),
    );
    menus.add(menu);
    ...
    if (widget.user.isAdmin == true) {
      Menu menu = Menu(
        ...
        assetV: 'assets/NewFondo.png',
        color: const Color.fromARGB(255, 1, 5, 17).withOpacity(0.7),
      );
      menus.add(menu);
      menu = Menu(
        nombre: 'Agregar Rollo',
        assetV: 'assets/iconNuevo.png',
        color: const Color.fromARGB(255, 4, 37, 88).withOpacity(0.7),
      );
      menus.add(menu);
      menu = Menu(
        nombre: 'Agregar Compra',
        assetV: 'assets/Factura.png',
        color: const Color.fromARGB(255, 6, 5, 75).withOpacity(0.7),
      );
      menus.add(menu);
      menu = Menu(
        nombre: 'Compras',
      ...
      menus.add(menu);
      menu = Menu(
        nombre: 'Entrada Almacen',
        assetV: 'assets/Almacen.png',
        color: const Color.fromARGB(248, 5, 51, 73).withOpacity(0.7),
      );
      menus.add(menu);
      menu = Menu(
        nombre: 'Revisar Inventario',
        assetV: 'assets/inventario.png',
        color: const Color.fromARGB(248, 5, 51, 73).withOpacity(0.7),
      );
      menus.add(menu);
    }
    ...
    menu = Menu(
      ...
      assetV: 'assets/Salir.png',
      color: const Color.fromARGB(255, 2, 11, 26).withOpacity(0.7),
    );
    menus.add(menu);
  }
}
